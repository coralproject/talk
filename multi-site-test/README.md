...
#### Enabling SSO Registration in Coral

Don't forget to enable `Allow Registration` in Coral under Login with Single Sign On or the SSO accounts won't work. This is usually on for our large SSO clients so it should be fine to use this while testing with `multi-site-test`.

![Enable SSO registration in Coral](readme/coral-allow-sso-registration.png)
