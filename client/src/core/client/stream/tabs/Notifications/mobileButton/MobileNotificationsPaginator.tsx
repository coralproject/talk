...
import styles from "./MobileNotificationsPaginator.css";
...
const MobileNotificationsPaginator: FunctionComponent<Props> = (props) => {
  ...
  const [, isRefetching] =
    useRefetch<MobileNotificationsPaginatorPaginationQueryVariables>(
      props.relay,
      10,
        ...
        {notificationsToShow.map(({ node }) => {
          return (
            <NotificationContainer
              key={`mobileNotificationsPaginator-${node.id}`}
              notification={node}
...
type FragmentVariables = MobileNotificationsPaginatorPaginationQueryVariables;

const enhanced = withPaginationContainer<
  Props,
  MobileNotificationsPaginatorPaginationQueryVariables,
  FragmentVariables
>(
  {
    query: graphql`
      fragment MobileNotificationsPaginator_query on Query
      @argumentDefinitions(
        ...
        notifications(ownerID: $viewerID, after: $cursor, first: $count)
          @connection(key: "MobileNotificationsPaginator_notifications") {
          edges {
            node {
              id
              type
  ...
  },
  {
    ...
    query: graphql`
      ...
      query MobileNotificationsPaginatorPaginationQuery(
        $viewerID: ID!
        $cursor: Cursor
        $count: Int!
      ) {
        ...MobileNotificationsPaginator_query
