-brand-name = The Coral Project
-product-name = Coral

common-banEmailTemplate =
  Hej { $username },
  Ktoś z dostępem do Twojego konta złamał zasady obowiązujące w naszej społeczności. W wyniku tego, Twoje konto zostało zbanowane. Nie możesz już więcej komentować, reagować i zgłaszać komentarzy.

common-embedNotFound = Nie znaleźliśmy tego pliku. Być może został usunięty.
common-networkError = Problem z połączeniem z siecią. Spróbuj odświeżyć okienko przeglądarki i otwórz ponownie.
