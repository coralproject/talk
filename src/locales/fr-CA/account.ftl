### Localization for Account

account-tokenNotFound =
  Le lien spécifié n'est pas valide, veuillez vérifier qu'il ait été copié correctement.

## Not Found

notFound = Non trouvé

## Password Reset

resetPassword-resetPassword = Réinitialiser le mot de passe
resetPassword-resetYourPassword = Réinitialiser votre mot de passe
resetPassword-pleaseEnterNewPassword =
  Veuillez entrer un nouveau mot de passe afin d'accéder à votre compte.
  Assurez-vous qu'il soit unique et gardez-le secret.
resetPassword-passwordLabel = Mot de passe
resetPassword-passwordDescription = Le mot de passe doit comporter au moins {$minLength} caractères.
resetPassword-passwordTextField =
  .placeholder = Mot de passe
resetPassword-successfullyReset = Votre mot de passe a bien été réinitialisé.
resetPassword-youMayClose =
  Vous pouvez fermer la fenêtre et vous connecter en utilisant votre nouveau mot de passe.
resetPassword-oopsSorry = Oups désolé!
resetPassword-missingResetToken = Le jeton de réinitialisation semble manquant.

## Email Confirmation

confirmEmail-emailConfirmation = Mail de confirmation
confirmEmail-confirmEmail = Confirmez votre courriel
confirmEmail-pleaseClickToConfirm =  Cliquez ci-dessous pour confirmer votre adresse courriel.
confirmEmail-oopsSorry = Oups désolé !
confirmEmail-missingConfirmToken = Le jeton de confirmation semble manquant.
confirmEmail-successfullyConfirmed = Votre courriel a été confirmé.
confirmEmail-youMayClose =
  Vous pouvez fermer cette fenêtre.

## Download

download-landingPage-title = Télécharger votre historique de commentaires.
download-landingPage-description =
  Votre historique de commentaires sera téléchargé sous la forme d'un dossier .zip.
  Après avoir dézippé votre historique de commentaires,
  vous aurez des valeurs séparées par une virgule (ou un fichier .csv)
  que vous pourrez facilement importer dans votre
  application tableur préférée.
download-landingPage-contentsDescription =
  Pour tous vos commentaires, les informations suivantes sont incluses :
download-landingPage-contentsDate =
  La date de votre commentaire
download-landingPage-contentsUrl =
  Le lien vers le commentaire
download-landingPage-contentsText =
  Le texte du commentaire
download-landingPage-contentsStoryUrl =
  Le lien de l'article où votre commentaire apparaît.
download-landingPage-downloadComments =
download-landingPage-download =
  Télécharge mon historique de commentaires.
download-landingPage-sorry = Votre lien de téléchargement n'est pas valide.

## Unsubscribe

unsubscribe-unsubscribeFromEmails = Se désinscrire des notifications par courriel.
unsubscribe-submit-unsubscribe = Se désinscrire
unsubscribe-youMayNowClose =
  Vous pouvez maintenant fermer cette fenêtre.
unsubscribe-oopsSorry = Oups désolé !
unsubscribe-unsubscribedSuccessfully =
unsubscribe-successfullyUnsubscribed = Vous ne recevrez désormais plus de notifications.
unsubscribe-clickToConfirm = Cliquez ci-dessous pour confirmer que vous ne voulez plus recevoir de notifications.
unsubscribe-confirm = Confirmer
