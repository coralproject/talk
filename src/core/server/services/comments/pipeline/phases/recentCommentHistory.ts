...
>): Promise<IntermediatePhaseResult | void> => {
  ...
  if (!tenant.ownSettings.recentCommentHistory.enabled) {
    return;
  }

  // Get the time frame.
  const since = DateTime.fromJSDate(now)
    .plus({ seconds: -tenant.ownSettings.recentCommentHistory.timeFrame })
    .toJSDate();
  ...
  if (rate >= tenant.ownSettings.recentCommentHistory.triggerRejectionRate) {
    return {
