...
import { CreateLocalContextParameters } from "coral-framework/lib/bootstrap/createManaged";
...
interface CreateContextFactoryParams {
  amp?: boolean;
}

const createContextBuilder = async ({ amp }: CreateContextFactoryParams) => {
  const createContext = async ({
    context,
    auth = null,
    accessToken,
  }: CreateLocalContextParameters) => {
    if (accessToken) {
    ...
    const StreamLocal: FunctionComponent = ({ children }) => {
      ...
      const [useAmp, setUseAmp] = useState<boolean>(Boolean(amp));
      ...
      const [authAccessTokenJTI, setAuthAccessTokenJTI] = useState<
        string | null
      >(auth?.claims.jti || null);
      const [authAccessTokenExp, setAuthAccessTokenExp] = useState<
        number | null
