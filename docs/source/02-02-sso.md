...
Coral provides a refresh access token mechanism which can be used to handle access tokens expiring. Configure the refresh access token callback on your stream embed:

```js
Coral.createStreamEmbed({
     ...options,
     refreshAccessToken: function(nextAccessToken) {
       const token = retrieveNewAccessToken();
       nextAccessToken(token);
     }
  });
```
If using Coral's "Login with email authentication" instead of SSO, the auth popup will open to let the user re-authenticate.
