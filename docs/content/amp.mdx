...
Then put the following code into your _AMP_ page and replace `{{ CORAL_DOMAIN_NAME }}` and `{{ CORAL_STORY_URL }}` with the
...
and live in a different domain than the `amp` page. For example: `src="https://{{ MY_AMP_PAGE }}?storyURL={{ CORAL_STORY_URL }}"`.
